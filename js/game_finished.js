...
const originalBoxSize = 1.5; // Original width and height of a box
let autopilot;
let gameEnded;
let robotPrecision; // Determines how precise the game is on autopilot

let startPos = {x: 4, y: 4, z: 0}
...
const autoPilotInfoHolderElement = document.querySelector(".auto-pilot");
...
function animation(time) {
  if (lastTime) {
    ...
    if (camera.position.y < boxHeight * (stack.length - 2) + 4) {
      lookAtPos.y = lookAtPos.y + speed * timePassed;
  ...
  let msg = "Autopilot: " + (autopilot? "ON" : "OFF");
  autoPilotInfoHolderElement.innerHTML = msg;
}
